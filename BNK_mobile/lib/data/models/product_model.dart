class ProductModel {
  ...
  factory ProductModel.fromJson(Map<String, dynamic> json) {
    return ProductModel(
      ...
      productType: (json['productType'] ?? json['product_type'] ?? '')
          .toString(),
      productStatus: (json['productStatus'] ?? json['product_status'] ?? '')
          .toString(),
      ...
      interestCalcType: (json['interestCalcType'] ?? json['interest_calc_type'])
          ?.toString(),
    );
  }
  ...
  String get joinMethodText {
    ...
    if (mobileJoinYn == 'Y') methods.add('모바일');
    if (branchJoinYn == 'Y') methods.add('영업점');
    return methods.isEmpty ? '가입채널 확인 필요' : methods.join(' · ');
  }
}
